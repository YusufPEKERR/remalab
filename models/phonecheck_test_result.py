...
class PhonecheckTestResult(Base):
    ...
    imei = Column(String(100), nullable=False, index=True)
    test_stage = Column(String(30), nullable=False)  # ILK_TEST (103->104) / SON_TEST (125->109)
    attempt_no = Column(Integer, nullable=True)  # SON_TEST icin, bu IMEI'nin kacinci basarisiz son test denemesi
    ...
    # Phonecheck'te cihaz bulunamadiginda elle doldurulan kayitlar
    is_manual = Column(Boolean, default=False)
    manual_reason = Column(Text, nullable=True)  # is_manual=True ise zorunlu
