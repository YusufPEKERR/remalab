  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      {isOpen && filtered.length > 0 && (
        <div className="absolute left-0 right-0 mt-1 bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl z-[150] max-h-60 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60">
          {filtered.map(opt => (
            <button
              key={opt}
              type="button"
              onClick={() => { onChange(opt); setIsOpen(false); }}
              className="w-full text-left px-3.5 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-[#1e222d] transition-colors"
            >
              {opt}
            </button>
          ))}
