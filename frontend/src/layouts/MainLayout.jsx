...
export default function MainLayout() {
  ...
  const allowedPaths = {
    ...
    'yönetici': ['/dashboard', '/depo', '/irsaliye', '/raporlar', '/parts', '/products', '/suppliers', '/locations', '/users', '/settings', '/quality', '/refurbishment', '/priority'],
  ...
  return (
    <div className="flex h-screen bg-slate-50 dark:bg-[#0f1219] text-slate-800 dark:text-slate-200 overflow-hidden transition-colors duration-200">
      {/* Sidebar */}
      <aside className="w-64 bg-white dark:bg-[#161B22] text-slate-600 dark:text-slate-300 flex flex-col border-r border-slate-200 dark:border-[#30363D] z-20 transition-colors duration-200">
        <div className="py-6 flex items-center justify-center px-4 border-b border-slate-200 dark:border-[#30363D] transition-colors duration-200">
          <img src="/logo.png" alt="RemaLab Logo" className="w-[180px] object-contain drop-shadow-md" />
        </div>
                    ...
                    <a
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                        isActive 
                          ...
                          : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-[#1e2330] hover:text-slate-800 dark:hover:text-slate-200'
                      }`}
                      onClick={(e) => {
                        e.preventDefault();
                        navigate(item.path);
        ...
        <div className="p-4 border-t border-slate-200 dark:border-[#30363D] transition-colors duration-200">
          <button 
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {/* Topbar */}
        <header className="h-16 bg-white dark:bg-[#161B22] border-b border-slate-200 dark:border-[#30363D] flex items-center justify-between px-6 shadow-sm z-50 relative shrink-0 transition-colors duration-200">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <div className="hidden lg:flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-[#1e2330] rounded-lg border border-slate-200 dark:border-slate-700/50">
                <span className="text-[11px] font-medium text-slate-500 dark:text-slate-400">⏱ SON GÜNCELLEME:</span>
                <span className="text-xs font-bold text-slate-700 dark:text-slate-200 font-mono tracking-wider">
              ...
              <button
                onClick={toggleTheme}
                className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-amber-500 dark:hover:text-amber-400 transition-colors bg-slate-100 dark:bg-[#1e2330] rounded-lg border border-slate-200 dark:border-slate-700/50"
              ...
              <button
                ...
                className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors bg-slate-100 dark:bg-[#1e2330] rounded-lg border border-slate-200 dark:border-slate-700/50 hover:border-blue-300 dark:hover:border-blue-500/50"
            ...
            <div className="relative">
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-3 w-[360px] bg-white dark:bg-[#1e2330] border border-slate-200 dark:border-slate-700 shadow-2xl rounded-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2">
                  <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-[#242a38] flex items-center justify-between">
                    <h3 className="font-bold text-slate-800 dark:text-slate-100">Kritik Stok Bildirimleri</h3>
                    {notifications.length > 0 && <span className="bg-red-500/20 text-red-600 dark:text-red-400 text-xs font-bold px-2 py-0.5 rounded-md">{notifications.length} Uyarı</span>}
                  </div>
            ...
            <div className="flex items-center gap-3 border-l border-slate-200 dark:border-[#30363D] pl-4">
              ...
              <div className="hidden md:block">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{(user && user.role) ? user.role : 'Guest'}</p>
              </div>
            </div>
          </div>
        </header>
