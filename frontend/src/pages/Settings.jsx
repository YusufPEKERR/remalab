          ...
          <div className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-6 shadow-sm">
            ...
            <div className="flex flex-wrap items-center justify-between p-4 bg-slate-50 dark:bg-[#181a24] rounded-xl border border-slate-200 dark:border-slate-700/50">
              ...
              <select
                ...
                className="bg-white dark:bg-[#12141c] border border-slate-300 dark:border-slate-600 text-slate-800 dark:text-slate-200 rounded-lg px-4 py-2.5 focus:outline-none focus:border-blue-500 font-medium"
              >
              ...
              <>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  {connections.map(conn => {
                    ...
                    return (
                      <div key={conn.id} className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 flex gap-5 hover:border-slate-500 transition-colors group relative overflow-hidden">
                        {conn.active && (
                        ...
                        <div className="flex flex-col gap-2 shrink-0 justify-center">
                          <button onClick={() => alert("Bağlantı test ediliyor...")} className="px-3 py-1.5 bg-slate-50 dark:bg-[#181a24] hover:bg-slate-100 dark:hover:bg-[#1e222d] text-slate-700 dark:text-slate-300 rounded-lg text-xs font-medium border border-slate-300 dark:border-slate-600 transition-colors flex items-center gap-1.5">
                ...
                <form onSubmit={handleSaveDb} className="space-y-5">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="text" required className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.name} onChange={e => setDbFormData({...dbFormData, name: e.target.value})} placeholder="├ûrn: Ana Veritabanı"/>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1.5">Veritabanı Türü</label>
                      <select className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.db_type} onChange={e => setDbFormData({...dbFormData, db_type: e.target.value})}>
                  ...
                  <div className="grid grid-cols-[2fr_1fr] gap-5">
                    <div>
                      ...
                      <input type="text" required className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.host} onChange={e => setDbFormData({...dbFormData, host: e.target.value})} placeholder="localhost"/>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1.5">Port</label>
                      <input type="number" required className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.port} onChange={e => setDbFormData({...dbFormData, port: Number(e.target.value)})} />
                    </div>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-slate-400 mb-1.5">Veritabanı Adı (Database)</label>
                    <input type="text" required className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.database} onChange={e => setDbFormData({...dbFormData, database: e.target.value})} placeholder="remalab_db"/>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1.5">Kullanıcı Adı</label>
                      <input type="text" required className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.username} onChange={e => setDbFormData({...dbFormData, username: e.target.value})} placeholder="postgres"/>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1.5">┼Şifre</label>
                      <input type="password" required className="w-full bg-slate-50 dark:bg-[#181a24] border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-slate-800 dark:text-slate-200 focus:outline-none focus:border-blue-500" value={dbFormData.password} onChange={e => setDbFormData({...dbFormData, password: e.target.value})} placeholder="ÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇó"/>
                    </div>
                  </div>

                  <div className="flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-700/50 mt-6">
                    <button type="button" onClick={() => setShowDbForm(false)} className="px-5 py-2.5 bg-slate-50 dark:bg-[#181a24] hover:bg-slate-100 dark:hover:bg-[#1e222d] text-slate-700 dark:text-slate-300 rounded-xl font-medium transition-colors border border-slate-300 dark:border-slate-600">─░ptal</button>
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  <button onClick={async () => { const res = await api.addLocalFile(); if(res.success) fetchLocalData(); else if(res.message !== "Seçim iptal edildi") alert(res.message); }} className="flex items-center gap-2 bg-slate-50 dark:bg-[#181a24] hover:bg-slate-100 dark:hover:bg-[#1e222d] text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-600 px-4 py-2 rounded-xl transition-all font-medium text-sm">
                    Var Olanı Ekle
                  </button>
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                {localFiles.map(file => (
                  <div key={file.id} className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 flex gap-4 hover:border-slate-500 transition-colors">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={async () => { const res = await api.addDataFolder(); if(res.success) fetchLocalData(); else if(res.message !== "Seçim iptal edildi") alert(res.message); }} className="flex items-center gap-2 bg-slate-50 dark:bg-[#181a24] hover:bg-slate-100 dark:hover:bg-[#1e222d] text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-600 px-4 py-2 rounded-xl transition-all font-medium text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {dataFolders.map(folder => {
                  ...
                  return (
                    <div key={folder.id} className="bg-white dark:bg-[#12141c] border border-slate-200 dark:border-slate-700/50 rounded-2xl p-4 flex gap-4 hover:border-slate-500 transition-colors items-center">
