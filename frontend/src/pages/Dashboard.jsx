...
export default function Dashboard() {
  ...
  return (
    <div className="flex flex-col space-y-6 pb-12 text-[#12141c] dark:text-slate-100 max-w-[1600px] mx-auto animate-in fade-in duration-300">
        ...
        {cards.map(card => {
          ...
          return (
            <div 
              ...
              className={`
                group relative bg-white dark:bg-[#12141c] rounded-2xl p-5 border transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md
                ${isSelected ? 'border-[#00b2ff] ring-2 ring-[#00b2ff]/20 bg-blue-50/20 dark:bg-blue-900/20' : 'border-[#DCE1F1] dark:border-[#1e222d] hover:border-blue-400 dark:hover:border-blue-500'}
              `}
            >
      ...
      {activeDetail && (
        <div className="bg-white dark:bg-[#12141c] rounded-2xl p-6 border border-[#DCE1F1] dark:border-[#1e222d] shadow-md animate-in fade-in slide-in-from-top-3 duration-200">
          <div className="flex flex-wrap items-center justify-between mb-4 border-b border-[#DCE1F1] dark:border-[#1e222d] pb-3">
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-[#00b2ff]" />
              <h3 className="text-base font-semibold text-[#12141c] dark:text-white">
                {cards.find(c => c.id === activeDetail)?.title} - Hızlı Detay Listesi
              </h3>
            </div>
            <button 
              onClick={() => setActiveDetail(null)}
              className="text-xs text-[#5A6685] dark:text-slate-400 hover:text-[#12141c] dark:hover:text-white font-semibold px-2.5 py-1 rounded-lg bg-[#EFF1FA] dark:bg-[#1e222d] border border-[#DCE1F1] dark:border-[#2e3545] transition-colors"
            >
              Kapat ✕
            </button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs whitespace-nowrap">
              <thead className="bg-[#F5F7FC] dark:bg-[#181a24] text-[#5A6685] dark:text-slate-400 font-semibold uppercase tracking-wider border-b border-[#DCE1F1] dark:border-[#1e222d]">
                <tr>
              ...
              <tbody className="divide-y divide-[#DCE1F1] dark:divide-[#1e222d]">
                {detailData.length === 0 ? (
                  ...
                  detailData.map((d, i) => (
                    <tr key={i} className="hover:bg-[#EEF1FB]/50 dark:hover:bg-[#1e222d]/70 text-[#12141c] dark:text-slate-200 transition-colors">
                      <td className="px-4 py-3 font-semibold text-[#12141c] dark:text-white">{d.name}</td>
                      <td className="px-4 py-3 font-mono text-[#5A6685] dark:text-slate-400">{d.code || '-'}</td>
                      <td className="px-4 py-3 font-medium text-[#2e3545] dark:text-slate-300">{d.loc || '-'}</td>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#12141c] rounded-2xl p-6 border border-[#DCE1F1] dark:border-[#1e222d] shadow-sm flex flex-col justify-between transition-colors">
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-[#DCE1F1] dark:border-[#1e222d]">
              <div>
                <h3 className="text-base font-semibold text-[#12141c] dark:text-white flex items-center gap-2">
                  <Activity size={18} className="text-[#00b2ff] dark:text-blue-400" /> Son Stok Hareketleri
                </h3>
              ...
              <button 
                ...
                className="inline-flex items-center gap-1 text-xs font-bold text-[#00b2ff] dark:text-blue-400 hover:text-blue-500 bg-[#EEF1FB] dark:bg-[#181a24] px-3 py-1.5 rounded-lg border border-blue-200 dark:border-[#1e222d] transition-colors self-start sm:self-auto"
              >
                ...
                <tbody className="divide-y divide-[#DCE1F1] dark:divide-[#1e222d]">
                  {recentMovements.length > 0 ? (
                    recentMovements.map((mov, i) => {
                      ...
                      return (
                        <tr key={mov.id || i} className="hover:bg-[#EEF1FB]/60 dark:hover:bg-[#1e222d]/70 text-[#12141c] dark:text-slate-200 transition-colors">
                          <td className="px-3.5 py-3.5 font-mono text-[#5A6685] dark:text-slate-400 font-semibold">#{mov.id}</td>
                          <td className="px-3.5 py-3.5 font-semibold text-[#12141c] dark:text-white max-w-[180px] truncate" title={mov.part_name}>{mov.part_name || '-'}</td>
                          <td className="px-3.5 py-3.5 text-[#2e3545] dark:text-slate-300 font-medium">
                          ...
                          <td className="px-3.5 py-3.5 font-semibold text-[#12141c] dark:text-white">{mov.quantity}</td>
          ...
          <div className="mt-4 pt-3 border-t border-[#DCE1F1] dark:border-[#1e222d] flex flex-wrap items-center justify-between text-xs text-[#5A6685] dark:text-slate-400">
            <span>Son 24 saatteki hareketler gösteriliyor</span>
            <span className="font-semibold text-[#00b2ff] dark:text-blue-400">Otomatik Canlı Güncelleme Açık</span>
          </div>
        </div>
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="bg-white dark:bg-[#12141c] rounded-2xl p-6 border border-[#DCE1F1] dark:border-[#1e222d] shadow-sm transition-colors">
            <h3 className="text-base font-semibold text-[#12141c] dark:text-white mb-1 flex items-center gap-2">
              <Zap size={18} className="text-[#00b2ff] dark:text-blue-400" /> Hızlı Erişim Kısayolları
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                ...
                <button
                  ...
                  className="flex flex-col p-3.5 rounded-xl border border-[#DCE1F1] dark:border-[#1e222d] hover:border-blue-300 dark:hover:border-blue-500 bg-[#F5F7FC] dark:bg-[#181a24] hover:bg-[#EEF1FB]/60 dark:hover:bg-blue-900/20 transition-all text-left group cursor-pointer"
                >
                  <div className="flex flex-wrap items-center justify-between mb-2">
                    <div className="w-8 h-8 rounded-lg bg-white dark:bg-[#12141c] border border-[#DCE1F1] dark:border-[#2e3545] flex items-center justify-center shadow-xs">
                      <sc.icon size={16} color={sc.color} />
                    </div>
                    <ChevronRight size={14} className="text-[#5A6685] dark:text-slate-400 group-hover:text-[#00b2ff] dark:group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all" />
                  </div>
                  <span className="text-xs font-bold text-[#12141c] dark:text-white">{sc.title}</span>
          ...
          <div className="bg-white dark:bg-[#12141c] rounded-2xl p-6 border border-[#DCE1F1] dark:border-[#1e222d] shadow-sm flex-1 flex flex-col justify-between transition-colors">
            <div>
              ...
              <div className="space-y-3">
                ...
                <div className="w-full bg-[#EFF1FA] dark:bg-[#181a24] h-2 rounded-full overflow-hidden">
                ...
                <div className="flex flex-wrap items-center justify-between text-xs pt-2">
                  ...
                  <span className="font-bold text-[#00b2ff] dark:text-blue-400">%98.4 Güvenli</span>
                </div>
                <div className="w-full bg-[#EFF1FA] dark:bg-[#181a24] h-2 rounded-full overflow-hidden">
                  <div className="bg-[#00b2ff] h-full rounded-full w-[98.4%]" />
                </div>
              </div>
            </div>

            <div className="mt-6 pt-4 border-t border-[#DCE1F1] dark:border-[#1e222d] text-[11px] text-[#5A6685] dark:text-slate-400 flex flex-wrap items-center justify-between">
              <span>Remalab WMS v1.0</span>
              <span className="font-semibold text-[#12141c] dark:text-white">Kesintisiz Bağlantı</span>
            </div>
          </div>
