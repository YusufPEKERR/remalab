...
export default function Dashboard() {
  ...
  const cards = [
    ...
    { 
      ...
      icon: <ArrowDownToLine size={22} color="#22C55E" /> 
    },
    { 
      id: 'outbound', 
      title: 'Bugünkü Çıkış', 
      ...
      color: '#2563EB', 
      bgColor: 'rgba(37,99,235,0.08)',
      icon: <ArrowUpFromLine size={22} color="#2563EB" /> 
    },
    { 
      id: 'locations', 
      ...
      icon: <MapPin size={22} color="#0284C7" /> 
    }
  ];

  return (
    <div className="flex flex-col space-y-6 pb-12 text-[#0F172A] dark:text-slate-100 max-w-[1600px] mx-auto animate-in fade-in duration-300">
        ...
        {cards.map(card => {
          ...
          return (
            <div 
              ...
              className={`
                group relative bg-white dark:bg-[#0F172A] rounded-2xl p-5 border transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md
                ${isSelected ? 'border-[#2563EB] ring-2 ring-[#2563EB]/20 bg-blue-50/20 dark:bg-blue-900/20' : 'border-[#E2E8F0] dark:border-[#1E293B] hover:border-blue-400 dark:hover:border-blue-500'}
              `}
            >
      ...
      {activeDetail && (
        <div className="bg-white dark:bg-[#0F172A] rounded-2xl p-6 border border-[#E2E8F0] dark:border-[#1E293B] shadow-md animate-in fade-in slide-in-from-top-3 duration-200">
          <div className="flex items-center justify-between mb-4 border-b border-[#E2E8F0] dark:border-[#1E293B] pb-3">
            ...
            <button 
              ...
              className="text-xs text-[#64748B] dark:text-slate-400 hover:text-[#0F172A] dark:hover:text-white font-semibold px-2.5 py-1 rounded-lg bg-[#F1F5F9] dark:bg-[#1E293B] border border-[#E2E8F0] dark:border-[#334155] transition-colors"
            >
              Kapat ✕
            </button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs whitespace-nowrap">
              ...
              <tbody className="divide-y divide-[#E2E8F0] dark:divide-[#1E293B]">
                {detailData.length === 0 ? (
                  <tr><td colSpan="6" className="px-4 py-6 text-center text-[#64748B] dark:text-slate-400">Detay verisi bulunamadı.</td></tr>
                ) : (
                  detailData.map((d, i) => (
                    <tr key={i} className="hover:bg-[#EFF6FF]/50 dark:hover:bg-[#1E293B]/70 text-[#0F172A] dark:text-slate-200 transition-colors">
                      ...
                      <td className="px-4 py-3 text-right">
                        <button onClick={() => navigate('/depo')} className="text-[#2563EB] dark:text-blue-400 hover:text-[#1D4ED8] font-bold inline-flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#0F172A] rounded-2xl p-6 border border-[#E2E8F0] dark:border-[#1E293B] shadow-sm flex flex-col justify-between transition-colors">
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-[#E2E8F0] dark:border-[#1E293B]">
              ...
              <button 
                ...
                className="inline-flex items-center gap-1 text-xs font-bold text-[#2563EB] dark:text-blue-400 hover:text-[#1D4ED8] bg-[#EFF6FF] dark:bg-blue-950/60 px-3 py-1.5 rounded-lg border border-blue-200 dark:border-blue-800/60 transition-colors self-start sm:self-auto"
              >
          ...
          <div className="mt-4 pt-3 border-t border-[#E2E8F0] dark:border-[#1E293B] flex items-center justify-between text-xs text-[#64748B] dark:text-slate-400">
            <span>Son 24 saatteki hareketler gösteriliyor</span>
            <span className="font-semibold text-[#2563EB] dark:text-blue-400">Otomatik Canlı Güncelleme Açık</span>
          </div>
        </div>
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="bg-white dark:bg-[#0F172A] rounded-2xl p-6 border border-[#E2E8F0] dark:border-[#1E293B] shadow-sm transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { title: 'Stok Listesi', path: '/depo', icon: Package, count: stats.totalParts || '0', color: '#2563EB' },
                { title: 'İş Emirleri', path: '/is-emirleri', icon: Layers, count: 'Aktif', color: '#0284C7' },
                { title: 'Servis Kayıtları', path: '/servis', icon: Activity, count: 'Takip', color: '#22C55E' },
                { title: 'Raporlar', path: '/raporlar', icon: FileText, count: 'Analiz', color: '#8B5CF6' }
              ].map((sc, i) => (
                <button
                  key={i}
                  onClick={() => navigate(sc.path)}
                  className="flex flex-col p-3.5 rounded-xl border border-[#E2E8F0] dark:border-[#1E293B] hover:border-blue-300 dark:hover:border-blue-500 bg-[#F8FAFC] dark:bg-[#1E293B] hover:bg-[#EFF6FF]/60 dark:hover:bg-blue-900/20 transition-all text-left group cursor-pointer"
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="w-8 h-8 rounded-lg bg-white dark:bg-[#0F172A] border border-[#E2E8F0] dark:border-[#334155] flex items-center justify-center shadow-xs">
                      <sc.icon size={16} color={sc.color} />
                    </div>
                    <ChevronRight size={14} className="text-[#64748B] dark:text-slate-400 group-hover:text-[#2563EB] dark:group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all" />
                  </div>
                  <span className="text-xs font-bold text-[#0F172A] dark:text-white">{sc.title}</span>
                  <span className="text-[10px] text-[#64748B] dark:text-slate-400 font-medium mt-0.5">{sc.count}</span>
                </button>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#64748B] dark:text-slate-400 font-medium">Veritabanı Senkronizasyonu</span>
                ...
                <div className="flex items-center justify-between text-xs pt-2">
                  <span className="text-[#64748B] dark:text-slate-400 font-medium">Kritik Stok Doluluk Oranı</span>
                  <span className="font-bold text-[#2563EB] dark:text-blue-400">%98.4 Güvenli</span>
                </div>
                <div className="w-full bg-[#F1F5F9] dark:bg-[#1E293B] h-2 rounded-full overflow-hidden">
                  <div className="bg-[#2563EB] h-full rounded-full w-[98.4%]" />
                </div>
              </div>
            </div>

            <div className="mt-6 pt-4 border-t border-[#E2E8F0] dark:border-[#1E293B] text-[11px] text-[#64748B] dark:text-slate-400 flex items-center justify-between">
              <span>Remalab WMS v1.0</span>
              <span className="font-semibold text-[#0F172A] dark:text-white">Kesintisiz Bağlantı</span>
            </div>
          </div>
