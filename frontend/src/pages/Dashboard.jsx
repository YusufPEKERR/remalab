...
export default function Dashboard() {
  ...
  const cards = [
    { 
      ...
      icon: <Package size={22} color="#3B4C93" /> 
    },
    { 
      id: 'low_stock', 
    ...
    { 
      ...
      color: '#3B4C93', 
      bgColor: 'rgba(68, 87, 165,0.08)',
      icon: <ArrowUpFromLine size={22} color="#3B4C93" /> 
    },
    { 
      id: 'locations', 
  ...
  return (
    <div className="flex flex-col space-y-6 pb-12 text-[#16204A] dark:text-slate-100 max-w-[1600px] mx-auto animate-in fade-in duration-300">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <button
              ...
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#3B4C93] hover:bg-[#2E3F78] text-white text-xs font-bold shadow-lg shadow-blue-600/30 transition-all transform hover:-translate-y-0.5 cursor-pointer"
            >
        ...
        {cards.map(card => {
          ...
          return (
            <div 
              ...
              className={`
                group relative bg-[#C6CEE2] dark:bg-[#16204A] rounded-2xl p-5 border transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md
                ${isSelected ? 'border-[#3B4C93] ring-2 ring-[#3B4C93]/20 bg-blue-50/20 dark:bg-blue-900/20' : 'border-[#8593B4] dark:border-[#24326A] hover:border-blue-400 dark:hover:border-blue-500'}
              `}
            >
              ...
              <div>
                <span className="text-xs font-medium text-[#2E3650] dark:text-slate-400 block mb-1">{card.title}</span>
                <div className="flex items-baseline justify-between">
                  <h3 className="text-2xl font-semibold text-[#16204A] dark:text-white tracking-tight">{card.value}</h3>
                  <span className="text-[11px] text-[#3B4C93] dark:text-blue-400 font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                    Detay &rsaquo;
                  </span>
                </div>
              </div>
      ...
      {activeDetail && (
        <div className="bg-[#C6CEE2] dark:bg-[#16204A] rounded-2xl p-6 border border-[#8593B4] dark:border-[#24326A] shadow-md animate-in fade-in slide-in-from-top-3 duration-200">
          <div className="flex flex-wrap items-center justify-between mb-4 border-b border-[#8593B4] dark:border-[#24326A] pb-3">
            <div className="flex items-center gap-2">
              <div className="w-2.5 h-2.5 rounded-full bg-[#3B4C93]" />
            ...
            <button 
              ...
              className="text-xs text-[#2E3650] dark:text-slate-400 hover:text-[#16204A] dark:hover:text-white font-semibold px-2.5 py-1 rounded-lg bg-[#9FADC9] dark:bg-[#24326A] border border-[#8593B4] dark:border-[#3B4A85] transition-colors"
            >
              Kapat ✕
            </button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs whitespace-nowrap">
              <thead className="bg-[#B5BFD8] dark:bg-[#1B2755] text-[#2E3650] dark:text-slate-400 font-semibold uppercase tracking-wider border-b border-[#8593B4] dark:border-[#24326A]">
                <tr>
              ...
              <tbody className="divide-y divide-[#8593B4] dark:divide-[#24326A]">
                {detailData.length === 0 ? (
                  <tr><td colSpan="6" className="px-4 py-6 text-center text-[#2E3650] dark:text-slate-400">Detay verisi bulunamadı.</td></tr>
                ) : (
                  detailData.map((d, i) => (
                    <tr key={i} className="hover:bg-[#EEF1FB]/50 dark:hover:bg-[#24326A]/70 text-[#16204A] dark:text-slate-200 transition-colors">
                      ...
                      <td className="px-4 py-3 font-mono text-[#2E3650] dark:text-slate-400">{d.code || '-'}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <button onClick={() => navigate('/depo')} className="text-[#3B4C93] dark:text-blue-400 hover:text-[#2E3F78] font-bold inline-flex items-center gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-[#C6CEE2] dark:bg-[#16204A] rounded-2xl p-6 border border-[#8593B4] dark:border-[#24326A] shadow-sm flex flex-col justify-between transition-colors">
          <div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-[#8593B4] dark:border-[#24326A]">
              <div>
                <h3 className="text-base font-semibold text-[#16204A] dark:text-white flex items-center gap-2">
                  <Activity size={18} className="text-[#3B4C93] dark:text-blue-400" /> Son Stok Hareketleri
                </h3>
                <p className="text-xs text-[#2E3650] dark:text-slate-400 mt-0.5">Sistemde gerçekleşen son 6 malzeme transfer ve giriş-çıkış işlemi</p>
              </div>
              <button 
                onClick={() => navigate('/raporlar')}
                className="inline-flex items-center gap-1 text-xs font-bold text-[#3B4C93] dark:text-blue-400 hover:text-[#2E3F78] bg-[#EEF1FB] dark:bg-blue-950/60 px-3 py-1.5 rounded-lg border border-blue-200 dark:border-blue-800/60 transition-colors self-start sm:self-auto"
              >
              ...
              <table className="w-full text-left text-xs whitespace-nowrap">
                ...
                <tbody className="divide-y divide-[#8593B4] dark:divide-[#24326A]">
                  {recentMovements.length > 0 ? (
                    recentMovements.map((mov, i) => {
                      ...
                      return (
                        <tr key={mov.id || i} className="hover:bg-[#EEF1FB]/60 dark:hover:bg-[#24326A]/70 text-[#16204A] dark:text-slate-200 transition-colors">
                          <td className="px-3.5 py-3.5 font-mono text-[#2E3650] dark:text-slate-400 font-semibold">#{mov.id}</td>
                          ...
                          <td className="px-3.5 py-3.5 text-[#2E3650] dark:text-slate-400 font-mono text-[11px]">{mov.created_at}</td>
                    ...
                    <tr>
                      <td colSpan="7" className="px-4 py-8 text-center text-[#2E3650] dark:text-slate-400">
          ...
          <div className="mt-4 pt-3 border-t border-[#8593B4] dark:border-[#24326A] flex flex-wrap items-center justify-between text-xs text-[#2E3650] dark:text-slate-400">
            <span>Son 24 saatteki hareketler gösteriliyor</span>
            <span className="font-semibold text-[#3B4C93] dark:text-blue-400">Otomatik Canlı Güncelleme Açık</span>
          </div>
        </div>
        ...
        <div className="flex flex-col space-y-6">
          ...
          <div className="bg-[#C6CEE2] dark:bg-[#16204A] rounded-2xl p-6 border border-[#8593B4] dark:border-[#24326A] shadow-sm transition-colors">
            <h3 className="text-base font-semibold text-[#16204A] dark:text-white mb-1 flex items-center gap-2">
              <Zap size={18} className="text-[#3B4C93] dark:text-blue-400" /> Hızlı Erişim Kısayolları
            </h3>
            <p className="text-xs text-[#2E3650] dark:text-slate-400 mb-4">Sık kullanılan modüllere doğrudan erişin</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { title: 'Stok Listesi', path: '/depo', icon: Package, count: stats.totalParts || '0', color: '#3B4C93' },
                ...
                <button
                  ...
                  className="flex flex-col p-3.5 rounded-xl border border-[#8593B4] dark:border-[#24326A] hover:border-blue-300 dark:hover:border-blue-500 bg-[#B5BFD8] dark:bg-[#24326A] hover:bg-[#EEF1FB]/60 dark:hover:bg-blue-900/20 transition-all text-left group cursor-pointer"
                >
                  <div className="flex flex-wrap items-center justify-between mb-2">
                    <div className="w-8 h-8 rounded-lg bg-[#C6CEE2] dark:bg-[#16204A] border border-[#8593B4] dark:border-[#3B4A85] flex items-center justify-center shadow-xs">
                      <sc.icon size={16} color={sc.color} />
                    </div>
                    <ChevronRight size={14} className="text-[#2E3650] dark:text-slate-400 group-hover:text-[#3B4C93] dark:group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all" />
                  </div>
                  <span className="text-xs font-bold text-[#16204A] dark:text-white">{sc.title}</span>
                  <span className="text-[10px] text-[#2E3650] dark:text-slate-400 font-medium mt-0.5">{sc.count}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#C6CEE2] dark:bg-[#16204A] rounded-2xl p-6 border border-[#8593B4] dark:border-[#24326A] shadow-sm flex-1 flex flex-col justify-between transition-colors">
            <div>
              ...
              <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between text-xs">
                  <span className="text-[#2E3650] dark:text-slate-400 font-medium">Veritabanı Senkronizasyonu</span>
                  <span className="font-bold text-emerald-600 dark:text-emerald-400">Aktif</span>
                </div>
                <div className="w-full bg-[#9FADC9] dark:bg-[#24326A] h-2 rounded-full overflow-hidden">
                  <div className="bg-emerald-500 h-full rounded-full w-full" />
                </div>

                <div className="flex flex-wrap items-center justify-between text-xs pt-2">
                  <span className="text-[#2E3650] dark:text-slate-400 font-medium">Kritik Stok Doluluk Oranı</span>
                  <span className="font-bold text-[#3B4C93] dark:text-blue-400">%98.4 Güvenli</span>
                </div>
                <div className="w-full bg-[#9FADC9] dark:bg-[#24326A] h-2 rounded-full overflow-hidden">
                  <div className="bg-[#3B4C93] h-full rounded-full w-[98.4%]" />
                </div>
              </div>
            </div>

            <div className="mt-6 pt-4 border-t border-[#8593B4] dark:border-[#24326A] text-[11px] text-[#2E3650] dark:text-slate-400 flex flex-wrap items-center justify-between">
