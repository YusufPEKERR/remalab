...
class StatCard(QWidget):
    ...
    def __init__(
        self, icon: str, title_key: str, value: str,
        change: str = "", change_positive: bool = True,
        accent_color: str = "#1F6FEB", parent=None
    ):
    ...
    def _setup_ui(
        self, icon: str, value: str,
        change: str, change_positive: bool, accent_color: str
    ):
...
class DashboardPage(QWidget):
    ...
    SAMPLE_MOVEMENTS = [
        ("MOV-2024-001", "iPhone 15 Pro LCD", "A-12-03", "movement.inbound", "156", "2 dk önce", "status.completed"),
        ("MOV-2024-002", "Samsung S24 Battery", "B-05-01", "movement.outbound", "89", "15 dk önce", "status.in_progress"),
        ("MOV-2024-003", "Pixel 8 Back Cover", "C-08-02", "movement.transfer", "45", "32 dk önce", "status.completed"),
        ("MOV-2024-004", "iPhone 14 Screen", "A-03-07", "movement.inbound", "200", "1 saat önce", "status.completed"),
        ("MOV-2024-005", "OnePlus 12 Charging Port", "D-11-04", "movement.outbound", "34", "2 saat önce", "status.pending"),
        ("MOV-2024-006", "Xiaomi 14 Camera Module", "B-02-09", "movement.inbound", "78", "3 saat önce", "status.completed"),
        ("MOV-2024-007", "Galaxy Z Fold5 Hinge", "E-01-01", "movement.transfer", "12", "4 saat önce", "status.in_progress"),
        ("MOV-2024-008", "iPad Air M2 Digitizer", "A-15-02", "movement.inbound", "65", "5 saat önce", "status.completed"),
    ]
    ...
    def _setup_ui(self):
        ...
        # Yenile Butonu
        self._refresh_btn = QPushButton(tr("dashboard.refresh"))
        self._refresh_btn.setObjectName("table_action_btn")
        self._refresh_btn.setCursor(Qt.PointingHandCursor)
        self._refresh_btn.clicked.connect(self.refresh_stats)
        header_layout.addWidget(self._refresh_btn)

        date_label = QLabel("📅  7 Temmuz 2026, Pazartesi")
        date_label.setStyleSheet("color: #8B949E; font-size: 13px;")
        ...
        for i, (icon, title_key, value, change, positive, color) in enumerate(self.CARD_DATA):
        ...
        self._db_section_title = QLabel(tr("dashboard.local_db_status"))
        self._db_section_title.setStyleSheet("color: #F0F6FC; font-size: 15px; font-weight: 600;")
        ...
        self._lbl_db_files_val.setStyleSheet("color: #F0F6FC; font-size: 13px; font-weight: bold;")
        ...
        self._lbl_sql_files_val.setStyleSheet("color: #F0F6FC; font-size: 13px; font-weight: bold;")
        ...
        self._lbl_db_size_val.setStyleSheet("color: #58A6FF; font-size: 13px; font-weight: bold;")
        ...
        self._lbl_active_db_val.setStyleSheet("color: #3FB950; font-size: 13px; font-weight: bold;")
        ...
        self._details_title_lbl.setStyleSheet("color: #58A6FF; font-size: 16px; font-weight: 600;")
    ...
    def _refresh_postgresql_stats(self):
        ...
        try:
            ...
            try:
                # 1. Total Parts
                total_parts = db.execute(text("SELECT COUNT(*) FROM warehouse.parts;")).scalar() or 0
                self._cards[0].set_value(f"{total_parts:,}")

                # 2. Total Stock
                total_stock = db.execute(text("SELECT SUM(quantity) FROM warehouse.stock;")).scalar() or 0
                self._cards[1].set_value(f"{total_stock:,}")

                # 3. Low Stock
                low_stock = db.execute(text("SELECT COUNT(*) FROM warehouse.stock WHERE quantity < 5;")).scalar() or 0
                self._cards[2].set_value(str(low_stock))

                # 4. Today's Inbound
                try:
                    todays_inbound = db.execute(text(
                ...
                # 5. Today's Outbound
                try:
                    todays_outbound = db.execute(text(
                        "SELECT SUM(quantity) FROM warehouse.stock_movements "
                ...
                # 6. Active Locations
                active_locations = db.execute(text("SELECT COUNT(*) FROM warehouse.locations;")).scalar() or 0
            ...
            QMessageBox.critical(
                self,
                tr("dashboard.db_error"),
                f"PostgreSQL bağlantı hatası:\n\n{str(e)[:300]}\n\nLütfen .env dosyanızı veya PostgreSQL sunucunuzu kontrol edin."
            )
    ...
    def _show_card_details(self, title_key: str):
        ...
        self._details_title_lbl.setText(tr("dashboard.details_title").format(name=tr(title_key)))
        ...
        try:
            ...
            try:
                if title_key == "dashboard.total_parts":
                    ...
                    table.setHorizontalHeaderLabels([tr("table.part_id"), tr("table.part_name")])
                    rows = db.execute(text("SELECT id, name FROM warehouse.parts ORDER BY id;")).fetchall()
                    table.setRowCount(len(rows))
                    for r_idx, row in enumerate(rows):
                        id_item = QTableWidgetItem(str(row[0]))
                        id_item.setFlags(id_item.flags() & ~Qt.ItemIsEditable)  # ID değiştirilemez
                ...
                elif title_key == "dashboard.total_stock" or title_key == "dashboard.low_stock":
                    table.setColumnCount(4)
                    table.setHorizontalHeaderLabels([tr("table.part_name"), tr("table.location"), tr("table.stock_quantity"), tr("table.status")])
                    query = """
                    ...
                    table.setColumnCount(2)
                    table.setHorizontalHeaderLabels([tr("table.location_id"), tr("table.location")])
                    rows = db.execute(text("SELECT id, name FROM warehouse.locations ORDER BY id;")).fetchall()
                    table.setRowCount(len(rows))
                    for r_idx, row in enumerate(rows):
                        id_item = QTableWidgetItem(str(row[0]))
                    ...
                    table.setColumnCount(4)
                    table.setHorizontalHeaderLabels([tr("table.movement_id"), tr("table.type"), tr("table.quantity"), tr("table.time")])
            ...
            QMessageBox.warning(self, tr("dashboard.db_error"), f"Detaylar yüklenirken hata oluştu:\n{e}")
        finally:
            ...
            try:
                if title_key == "dashboard.total_parts":
                    ...
                    try:
                        ...
                        QMessageBox.warning(self, "Hata", "Lütfen geçerli bir sayısal stok değeri girin.")
                        self.refresh_stats()
                        return
                elif title_key == "dashboard.active_locations":
                    db.execute(
                    ...
                    try:
                        qty = int(new_val)
                        db.execute(
                            text("UPDATE warehouse.stock_movements SET quantity = :qty WHERE id = :id;"),
                            {"qty": qty, "id": db_id}
                        )
                    except ValueError:
                        QMessageBox.warning(self, "Hata", "Lütfen geçerli bir sayısal değer girin.")
                        self.refresh_stats()
                        return
    ...
    def _update_table_headers(self):
        ...
        headers = [
            tr("table.movement_id"), tr("table.part_name"),
            tr("table.location"), tr("table.type"),
            tr("table.quantity"), tr("table.time"), tr("table.status")
        ]
