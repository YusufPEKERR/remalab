...
class AddLocationDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        buttons.button(QDialogButtonBox.Ok).setStyleSheet("background-color: #238636; color: white; padding: 6px 12px; border-radius: 4px;")
...
class LocationsPage(QWidget):
    ...
    def _update_headers(self):
        self._table.setHorizontalHeaderLabels([
            tr("table.location_id"),
            tr("locations.location_name"),
            tr("db.delete")
        ])
        ...
        try:
            ...
            try:
                ...
                for r_idx, row in enumerate(rows):
                    ...
                    del_btn.setStyleSheet("background-color: transparent; font-size: 14px; border: none;")
                    del_btn.setCursor(Qt.PointingHandCursor)
                    del_btn.clicked.connect(lambda checked, l_id=row[0]: self._delete_location(l_id))
    ...
    def _add_location(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                db = SessionLocal()
                try:
                    db.execute(text("INSERT INTO warehouse.locations (name) VALUES (:name);"), {"name": name})
                    db.commit()
                finally:
                    db.close()
                self._load_locations()
    ...
    def _delete_location(self, loc_id: int):
        ...
        reply = QMessageBox.question(
            self, tr("db.delete"), tr("locations.confirm_delete"),
        ...
        if reply == QMessageBox.Yes:
            try:
                ...
                db = SessionLocal()
                try:
                    db.execute(text("DELETE FROM warehouse.locations WHERE id = :id;"), {"id": loc_id})
                    db.commit()
                finally:
                    db.close()
                self._load_locations()
