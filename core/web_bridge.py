...
class WebBridge(QObject):
    ...
    @Slot(str, str, str, str, str, str, result=str)
    def add_inbound_entry(self, part_id, loc_id, qty, price, type_str, username):
        from models.stock import Stock
        from models.stock_movement import StockMovement
        db = SessionLocal()
        try:
            ...
            target_stock = db.query(Stock).with_for_update().filter(Stock.part_id == part_id, Stock.location_id == loc_id).first()
            if target_stock:
                target_stock.quantity += qty
            else:
                target_stock = Stock(part_id=part_id, location_id=loc_id, quantity=qty)
                db.add(target_stock)
            ...
            movement = StockMovement(
                ...
                created_by=username,
                description=f"Fiyat: {price}" if price else ""
            )
            db.add(movement)
            db.commit()
            return json.dumps({"success": True})
    ...
    @Slot(str, str, str, str, str, str, str, result=str)
    def add_outbound_entry(self, part_id, loc_id, qty, type_str, username, technician, description):
        from models.stock import Stock
        from models.stock_movement import StockMovement
        db = SessionLocal()
        try:
            ...
            source_stock = db.query(Stock).with_for_update().filter(Stock.part_id == part_id, Stock.location_id == loc_id).first()
            if not source_stock or source_stock.quantity < qty:
            ...
            desc_parts = []
            if technician:
                desc_parts.append(f"Teknisyen: {technician}")
            if description:
